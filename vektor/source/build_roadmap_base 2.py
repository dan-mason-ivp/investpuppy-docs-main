...
OUT = os.path.join(os.path.dirname(__file__), '..', 'output', 'ip-investor-memo.pdf')
...
def revenue_table():
    ...
    rows_data = [
        ...
        ('EBITDA',          '($494K)','($461K)','$154K', '$627K', '$1,477K'),
        ('EBITDA margin',   'n/m',   'n/m',   '18%',    '42%',   '59%'),
    ]
    data = [[Paragraph(h, ST['label']) for h in headers]]
...
def build_story():
    ...
    SP()
    rows = [
        ('Raise',           'US$1.5M – US$2.25M'),
        ('Pre-money',       'US$6M – US$10M'),
    ...
    NOTE(f'{B("Note:")} Founder names are placeholders in this version of the memo. '
         f'Full team biographies are available in the NDA version.')

    SP(8)
    H3('CEO')
    P(f'{B("[CEO name]")} — Co-equal founder. '
      f'Spent [X] years in institutional wealth management and technology '
      f'implementation across [institution types], with direct experience of the '
      f'operational failures the Unvarnished series documents. '
      f'Holds a production OpenWealth API implementation credential across Tier 1 '
      f'custodian institutions in Europe, the Middle East, and Asia Pacific — '
      f'a live relationship infrastructure that is the commercial foundation of '
      f'the Vektor : Wrapped channel. Disclosed in full under NDA.')

    SP(8)
    H3('CTO')
    P(f'{B("[CTO name]")} — Co-equal founder. Architect of the Vektor platform. '
      f'[X] years building systematic portfolio construction and quantitative methods '
      f'at [institution type]. '
      f'Responsible for the platform\'s core methodology: the signal generation engine, '
      f'the 10,000-configuration optimisation framework, and the audit trail architecture '
      f'that makes every decision transparent to regulators and clients. '
    ...
    SP(6)
    NOTE(f'{B("The CEO credential:")} Production OpenWealth API implementation across Tier 1 custodian '
         f'institutions in Europe, the Middle East, and Asia Pacific — disclosed under NDA. '
         f'This is not a planned integration. It is a live credential built over years of institutional relationship work. '
         f'It means InvestPuppy can initiate the Wrapped channel conversation at the infrastructure level '
         f'before most competitors can get a meeting with the right counterpart. '
    ...
    dist = [
        ...
        f'{B("DIFC / UAE:")} On first qualifying conversation.',
    ]
    for d in dist:
        BUL(d)
    ...
    market_facts = [
        ...
        'Switzerland: established EAM and IAM market with strong OpenWealth infrastructure adoption. Near-term expansion.',
    ...
    P(f'Revenue begins with Proof Partners and first Entry clients in Year 1. '
      ...
      f'Base case reaches EBITDA positive in Year 3. '
    ...
    proceeds = [
        ...
        (f'{B("Runway")}',                                   '18–24 months to first House commercial'),
    ]
    ...
    raise_data = [
        ('Target',           'US$1.5M – US$2.25M'),
        ('Pre-money',        'US$6M – US$10M (defensible with two-channel story and team credentials)'),
        ...
        ('Compatibility',    'Stealth-compatible · founders currently employed'),
        ('Use of proceeds',  '5-person team · institutional sales runway · legal infrastructure · product build'),
        ('Runway',           '18–24 months to first House commercial · first Wrapped partner active'),
        ('Milestones',       'First Proof Partner → first House Stage 2 → first Wrapped commercial'),
    ]
    ...
    P('The ideal investor for this round is a boutique wealth practitioner — '
      ...
      'more than capital — they bring the most valuable thing a pre-revenue platform '
      'can have: a credible, operationally experienced voice within the target market '
      'with a direct stake in the platform\'s success. InvestPuppy is not simply '
      'seeking capital. It is seeking investors who are also, in the fullest sense, '
      'constituents of the market it serves.')
    ...
    SP(10)
    NOTE(f'{B("Exit path:")} As Vektor scales across multiple deployment models and the OpenWealth '
         f'credential translates into live Wrapped partnerships, InvestPuppy will attract '
         f'acquisition interest from custodian banks, wealth management platforms, and '
         f'institutional portfolio management software consolidators — the same organisations '
         f'that currently pay US$200,000–US$500,000 per year for the infrastructure Vektor replaces. '
         f'Trade sale is the primary exit scenario. Timeline aligns with Year 4–5 revenue maturity.')
