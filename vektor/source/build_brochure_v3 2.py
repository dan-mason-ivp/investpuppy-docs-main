...
def build():
    ...
    story.append(p('The example throughout this document uses a Singapore equity mandate (SGX, SGD). The platform is market-agnostic: configure your exchange, benchmark, currency, and lot sizes \u2014 and the engine does the rest.',S_grey))
    ...
    st_rows=[
        ...
        [p('Strategy creation & optimisation',S_live),p('Live account execution (IBKR)',S_next),p('Multiple markets & currencies',S_scale)],
        [p('XGBoost signal validation',S_live),p('Real-time position updates post-fill',S_next),p('Any listed equity exchange',S_scale)],
        [p('Client onboarding & portfolio setup',S_live),p('Live P&L & performance dashboards',S_next),p('Automated rebalancing',S_next)],
        [p('Asset allocation at live prices',S_live),p('Walk-forward strategy validation',S_next),p('External PMS integration',S_next)],
        [p('Cash funding & full audit trail',S_live),p('Infrastructure RBAC / maker-checker',S_next),p('SAA & model portfolios',S_scale)],
        [p('Order preparation, queuing & Slack notification',S_live),p('',S_live),p('',S_scale)],
    ]
    ...
    story.append(p('Enquire about Founding Mandate availability at investpuppy.com',
    ...
    story.append(sp(6))
    story.append(p('For methodology: Vektor Research Series (WP-01 through WP-10)  \u00b7  For Founding Mandate details: vektor-founding-mandate-programme.pdf (IP-FMP-260501-1.0)  \u00b7  investpuppy.com',S_xref))
    story.append(sp(6))
