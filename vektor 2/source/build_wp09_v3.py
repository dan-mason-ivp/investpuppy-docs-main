...
def build():
    ...
    key_takeaways_box([
        ...
        'Proof Partners are invited to raise any risk not addressed here. The Proof Partners programme is a partnership in which both parties are building toward the same outcome \u2014 a platform that performs as described, with the governance and controls that institutional management requires.',
    ],s),sp(10),
    ...
    p("It is not a legal disclaimer. It does not attempt to limit liability through exhaustive caveats. Readers seeking a legal risk disclosure should request the separate legal documentation available to Proof Partners.",s['body']),
    NoteBox(COL_W,
    ...
    risk_rows=[
        ...
        [p('Simulation vs live gap',s['tbl_body']),p('Medium',s['tbl_body']),p('High',s['tbl_body']),
         p('Current IBKR connection is simulation account. Full order flow tested in simulation. Live execution before first Proof Partner onboarding.',s['tbl_body'])],
    ...
    p('The IBKR Gateway is currently connected to the IBKR simulation account. Simulation mode does not fully replicate live execution: fills are assumed at the requested price without market impact, partial fills do not occur, and liquidity is not modelled. The full order flow is operational and tested in simulation. Live execution will be activated before the first Proof Partner is onboarded \u2014 this is a defined trigger condition, not an open-ended roadmap item.',s['body']),
    ...
    p('ECS microservices on Fargate restart automatically on failure. RDS has automated daily backups. The SQS queue persists market orders through application restarts. No formal SLA is committed at the current stage. Production observability alerting is on the near-term roadmap. Availability commitments for Proof Partners will be agreed as part of the mandate terms.',s['body']),
    p('Regulatory compliance',s['wp_sub']),
    p('Vektor is a portfolio construction and management platform. Regulatory obligations \u2014 licensing, client suitability, reporting \u2014 remain with the portfolio manager and the firm operating the platform. Vektor provides the audit trail and compliance documentation described in WP-06. Full details of regulatory standing are available to Proof Partners on request.',s['body']),
    PageBreak(),sp(0),
    ...
    roadmap_rows=[
        ...
        [p('Live IBKR execution',s['tbl_gold']),p('Execution \u2014 simulation vs live gap',s['tbl_body']),p('Before first Proof Partner onboards',s['tbl_body'])],
        ...
        [p('Bloomberg / institutional data feed',s['tbl_body']),p('Data \u2014 market data source reliability',s['tbl_body']),p('Available as configuration for Proof Partners',s['tbl_body'])],
    ]
    ...
    NoteBox(COL_W,
        'The Proof Partners programme is a partnership in which both parties are building toward the same outcome \u2014 '
        'a platform that performs as described, with the governance and controls that institutional management '
        'requires. Proof Partners are invited to raise any risk they do not see addressed in this '
