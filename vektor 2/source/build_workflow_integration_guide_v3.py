...
story.append(wf_table([
    ...
    ('Submit orders to the broker', 'Replaces (when live)',
     'Orders submitted via SQS queue to IBKR Gateway. Currently in simulation mode \u2014 live execution before first Proof Partner onboards.'),
    ...
    ('Report to clients on portfolio performance', 'Not yet in scope',
     'Client reporting is on the product roadmap. Performance data available via API and Admin Portal. Formatted client reports are a near-term roadmap item.'),
], s))
story.append(sp(12))
...
story.append(p(
    ...
    'this is a configuration change, not an architectural change, and is available as a Proof Partner engagement option. '
...
story.append(p('A standard REST API integration between Vektor\u2019s allocation endpoint and an existing PMS typically takes two to four weeks for a developer familiar with the target PMS. Vektor provides API documentation and integration support as part of Proof Partners programme onboarding.', s['body']))
...
story.append(NoteBox(COL_W,
    ...
    'Partner engagement structure is designed to support this transition: Vektor is configured '
